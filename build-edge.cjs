...
const lines = [
  ...
  "      }",
  "      const MAX_VALUE_SIZE = 3 * 1024 * 1024; // 3MB 限制（边缘函数请求体限制）",
  "      const valueStr = typeof value === 'string' ? value : JSON.stringify(value);",
  "      if (valueStr.length > MAX_VALUE_SIZE) {",
  "        return new Response(JSON.stringify({status:0, error:'Value too large (max 3MB)'}), {status:413, headers: {...CORS, 'Content-Type':'application/json'}});",
  "      }",
