...
const lines = [
  ...
  "        const wKey = kvKey('__writes__' + today);",
  "        const wCount = Number(await TEXTDB.get(wKey) || '0') + 1;",
  "        await TEXTDB.put(wKey, String(wCount));",
  "      } catch (_) { /* 非关键操作 */ }",
  ...
  "      do {",
  "        const result = await TEXTDB.list({ prefix: KV_PREFIX, limit, cursor });",
  "        totalKeys += result.keys.length;",
  "        // 累加每个 value 的大小",
  "        for (const { key } of result.keys) {",
  "          try {",
  "            const val = await TEXTDB.get(key);",
  "            if (val) totalSize += (typeof val === 'string' ? val.length : JSON.stringify(val).length);",
  "          } catch (_) { /* skip errors */ }",
  "        }",
  "        cursor = result.cursor;",
  "        if (totalKeys >= MAX_SCAN_KEYS) { cursor = undefined; }",
  "      } while (cursor);",
  "      // 今日写入计数",
  "      const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD",
  "      const writesToday = Number(await TEXTDB.get(kvKey('__writes__' + today)) || '0');",
  "      return new Response(JSON.stringify({status:1, data:{totalKeys, totalSize, writesToday, scannedAll: totalKeys < MAX_SCAN_KEYS}}), {headers:{'Content-Type':'application/json', ...CORS}});",
  "    } catch (e) {",
